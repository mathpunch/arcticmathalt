	...
	if (event.key === 'Enter') {
		...
		fetchDomains().then(domains => {
			...
			if (/^(https?|ftp):\/\/[^\s/$.?#].[^\s]*$/.test(inputUrl)) {
				...
				document.getElementById('siteurl').src = scope + Ultraviolet.codec.xor.encode(inputUrl.includes('.') ? 'https://' + inputUrl : 'https://duckduckgo.com/?t=h_&ia=web&q=' + encodeURIComponent(inputUrl));
			}
		});
	}
});
